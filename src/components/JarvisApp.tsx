  ...
  return <div className="app-shell"><SmoothScroll />
    ...
    <main className={`main-content view-${view} ${messages.length ? 'has-messages' : ''}`}>
      <header className="prototype-nav-row">
        ...
        <div className="language-switch nav-language" aria-label={locale === 'ru' ? 'Язык' : 'Тіл'}><button onClick={() => changeLocale('ru')} className={locale === 'ru' ? 'active' : ''} aria-pressed={locale === 'ru'}>Рус</button><button onClick={() => changeLocale('kk')} className={locale === 'kk' ? 'active' : ''} aria-pressed={locale === 'kk'}>Қаз</button></div>
      </header>
