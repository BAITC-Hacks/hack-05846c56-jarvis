...
import AccountHistory, { type AccountHistoryHandle } from './account/AccountHistory';
...
export default function JarvisApp({ initialView = 'assistant' }: { initialView?: View }) {
  ...
  const setMessages = useCallback((value: DisplayMessage[] | ((current: DisplayMessage[]) => DisplayMessage[])) => {
    const next = typeof value === 'function' ? value(messagesRef.current) : value;
    messagesRef.current = next; setMessagesState(next);
  }, []);
  const [conversationId, setConversationIdState] = useState<string | null>(null);
  const conversationIdRef = useRef<string | null>(null);
  const setConversationId = useCallback((id: string | null) => { conversationIdRef.current=id; setConversationIdState(id); }, []);
  const historyRef = useRef<AccountHistoryHandle>(null);
  const [historyTransitioning,setHistoryTransitioning] = useState(false);
  const historyTransitionRef = useRef(false);
  const setHistoryTransition = useCallback((value:boolean) => { historyTransitionRef.current=value;setHistoryTransitioning(value); }, []);
  const getHistorySnapshot = useCallback(() => ({messages:messagesRef.current,conversationId:conversationIdRef.current}),[]);
  ...
  useEffect(() => {
    try {
      ...
      if (history) { const parsed = JSON.parse(history); if (Array.isArray(parsed)) setMessages(parsed.slice(-100)); }
      const activeId=sessionStorage.getItem('jarvis-conversation-id');
      if(activeId && /^[a-f0-9-]{36}$/i.test(activeId))setConversationId(activeId);
  ...
  useEffect(() => {
    if (hydrated) try { sessionStorage.setItem('jarvis-chat', JSON.stringify(messages.slice(-100))); } catch { /* A long chat still works without persistence. */ }
  }, [messages, hydrated]);
  useEffect(() => { if(hydrated)try{if(conversationId)sessionStorage.setItem('jarvis-conversation-id',conversationId);else sessionStorage.removeItem('jarvis-conversation-id');}catch{/* Optional session pointer. */} },[conversationId,hydrated]);
  useEffect(() => {
  ...
  async function sendMessage(text = input, context?: Product[], retry?: FailedRequest) {
    if (historyTransitionRef.current || sending || uploading || (!text.trim() && !attachments.length && !retry?.files.length)) return;
    const originalContent = text.trim() || (locale === 'ru' ? 'Изучи прикреплённые файлы и помоги подобрать товары из каталога.' : 'Тіркелген файлдарды зерттеп, каталогтан тауар таңдауға көмектес.');
    const content = redactPaymentDetails(originalContent);
    const originalFiles = retry?.files || attachments;
    const files = originalFiles.map(file => ({...file,name:redactPaymentDetails(file.name),text:file.text === undefined ? undefined : redactPaymentDetails(file.text),warning:file.warning === undefined ? undefined : redactPaymentDetails(file.warning)}));
    if(content !== originalContent || files.some((file,index)=>file.name!==originalFiles[index].name || file.text!==originalFiles[index].text || file.warning!==originalFiles[index].warning))setToast(locale === 'ru' ? 'Платёжные реквизиты скрыты в тексте перед отправкой и сохранением.' : 'Төлем деректері жіберу және сақтау алдында мәтінде жасырылды.');
  ...
  function requestNew() { if(!historyTransitionRef.current)void historyRef.current?.requestNew(); }
  function guestNew() { if(messagesRef.current.length)setResetConfirm(true);else reset(); }
  function beforeHistoryTransition() {
    if(uploading || cartBusy){setToast(locale==='ru'?'Дождитесь завершения загрузки или операции с корзиной.':'Жүктеу немесе себет әрекетінің аяқталуын күтіңіз.');return false;}
    abortRef.current?.abort();requestId.current++;setSending(false);return true;
  }
  ...
  return <div className="app-shell"><SmoothScroll enabled={view !== 'assistant' || messages.length === 0} /><div className="site-dot-background"><OriginalDotField /></div>
    ...
    <main className={`main-content view-${view} ${messages.length ? 'has-messages' : ''}`}>
      <header className="prototype-nav-row">
        ...
        <div className="nav-actions"><div className="language-switch nav-language" aria-label={locale === 'ru' ? 'Язык' : 'Тіл'}><button onClick={() => changeLocale('ru')} className={locale === 'ru' ? 'active' : ''} aria-pressed={locale === 'ru'}>Рус</button><button lang="kk" onClick={() => changeLocale('kk')} className={locale === 'kk' ? 'active' : ''} aria-pressed={locale === 'kk'}>Қаз</button></div><SpecularButton className="nav-start" borderRadius={10} disabled={historyTransitioning} onClick={requestNew}>{locale === 'ru' ? 'Начать подбор' : 'Таңдауды бастау'}</SpecularButton></div>
      </header>
      <div className="account-toolbar"><AccountHistory ref={historyRef} locale={locale} messages={messages} conversationId={conversationId} getSnapshot={getHistorySnapshot} onConversationIdChange={setConversationId} onRestore={restoreHistory} onNew={reset} onGuestNew={guestNew} onBeforeTransition={beforeHistoryTransition} onTransitionChange={setHistoryTransition} /></div>
    ...
    {resetConfirm && <Modal title={t.resetTitle} closeLabel={t.close} onClose={() => setResetConfirm(false)}><p className="modal-description">{locale==='ru'?'Вы не вошли в аккаунт. Эта переписка доступна только в текущей вкладке и не попадёт в облачную историю. Войдите, чтобы сохранять диалоги, или начните новый без сохранения.':'Аккаунтқа кірмедіңіз. Бұл әңгіме тек осы қойындыда қолжетімді және бұлт тарихына сақталмайды. Диалогтарды сақтау үшін кіріңіз немесе сақтамай жаңасын бастаңыз.'}</p><div className="modal-actions"><button className="secondary-button" onClick={() => setResetConfirm(false)}>{t.cancel}</button><button className="primary-button" onClick={reset}>{locale==='ru'?'Начать без сохранения':'Сақтамай бастау'}</button></div></Modal>}
