...
export default function OriginalDotField({className=''}:OriginalDotFieldProps){
  ...
  useEffect(()=>{
    ...
    function doResize(){const rect=container!.getBoundingClientRect(),w=rect.width,h=rect.height;canvas!.width=w*dpr;canvas!.height=h*dpr;canvas!.style.width=w+'px';canvas!.style.height=h+'px';ctx.setTransform(dpr,0,0,dpr,0,0);size={w,h,offsetX:rect.left,offsetY:rect.top};buildDots(w,h);if(motion.matches)draw();}
    function resize(){clearTimeout(resizeTimer);resizeTimer=setTimeout(doResize,100);}
    function onMouseMove(e:MouseEvent){if(motion.matches)return;mouse.x=e.clientX-size.offsetX;mouse.y=e.clientY-size.offsetY;}
    function onTouchMove(e:TouchEvent){if(motion.matches)return;if(e.touches&&e.touches[0]){mouse.x=e.touches[0].clientX-size.offsetX;mouse.y=e.touches[0].clientY-size.offsetY;}}
