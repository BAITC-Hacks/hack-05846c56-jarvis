...
export default function SmoothScroll({enabled=true}:{enabled?:boolean}){
  useEffect(()=>{if(!enabled)return;gsap.registerPlugin(ScrollTrigger);const motion=window.matchMedia('(prefers-reduced-motion: reduce)');let lenis:Lenis|undefined;const targetWindow=window as Window&{__lenis?:Lenis};const update=(time:number)=>lenis?.raf(time*1000);
