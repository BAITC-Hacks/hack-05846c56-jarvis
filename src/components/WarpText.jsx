...
const WarpText = ({
  ...
  useEffect(() => {
    ...
    const onPointerMove = event => {
      ...
      if (rect.width <= 0 || rect.height <= 0) return;
      pointer.tx = event.offsetX / canvas.clientWidth;
      pointer.ty = 1 - event.offsetY / canvas.clientHeight;
    ...
    const onVisibility = () => {
      ...
      if (pageVisible && visible && !reduceMotion && !raf) raf = requestAnimationFrame(loop);
      if (!pageVisible && raf) {
    ...
    const onReducedMotion = event => {
      ...
      if (reduceMotion && raf) { cancelAnimationFrame(raf); raf = 0; }
      program.uniforms.uPointerActive.value = 0;
      renderOnce();
      if (!reduceMotion && visible && pageVisible && !raf) raf = requestAnimationFrame(loop);
    };
    ...
    intersectionObserver = new IntersectionObserver(
      ([entry]) => {
        ...
        if (visible && pageVisible && !reduceMotion && !raf) raf = requestAnimationFrame(loop);
        if (!visible && raf) {
          cancelAnimationFrame(raf);
          raf = 0;
        }
  ...
  return (
    <div ref={containerRef} className={`warp-text ${className}`.trim()} style={style} role="img" aria-label={text}><span className="warp-text__fallback" aria-hidden="true">{text}</span></div>
  );
};
