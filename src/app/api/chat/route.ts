...
export async function POST(request:Request){
 ...
 try{const raw=await request.text();if(raw.length>4400000)return NextResponse.json({error:'Слишком большой запрос'},{status:413});const parsed=schema.safeParse(JSON.parse(raw));if(!parsed.success)return NextResponse.json({error:'Проверьте формат сообщения / Хабарлама пішімін тексеріңіз'},{status:400});return NextResponse.json(await answerAssistant(parsed.data));}catch{return NextResponse.json({error:'Не удалось обработать запрос. Попробуйте ещё раз. / Сұрауды өңдеу мүмкін болмады. Қайталап көріңіз.'},{status:500});}
}
