...
export async function POST(request:Request){
 ...
 if(ext==='pdf'){if(buffer.subarray(0,5).toString()!=='%PDF-')return NextResponse.json({error:`Некорректный PDF: ${name}`},{status:400});const {PDFParse}=await import('pdf-parse');const parser=new PDFParse({data:buffer});try{const info=await parser.getInfo();const pages=Array.from({length:Math.min(info.total,20)},(_,i)=>i+1);text=(await parser.getText({partial:pages})).text;if(info.total>20)text+='\n[Прочитаны первые 20 страниц]';}finally{await parser.destroy();}}
